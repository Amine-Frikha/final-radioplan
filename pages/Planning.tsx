...
const Planning: React.FC = () => {
  ...
  const handleResolve = (slotId: string, newDoctorId: string) => {
    // If newDoctorId is empty string, it means "Reset/Delete Override"
    if (newDoctorId === "") {
  ...
  // --- PDF GENERATION (Optimized for full capture) ---
  const handleDownloadPDF = async () => {
      ...
      try {
          ...
          // 2. CREATE A DEDICATED PRINT CONTAINER
          // We render off-screen but with a fixed large width to ensure no scrollbars and no cutoff.
          const printContainer = document.createElement('div');
          printContainer.style.position = 'absolute';
          printContainer.style.left = '-9999px';
          printContainer.style.top = '0';
          // A4 Landscape is roughly 297mm x 210mm. 
          // At 96 DPI, that's approx 1123px x 794px.
          // We double it for quality and to fit content: 2200px width.
          ...
          // 4. CLONE TABLE & STYLE FOR PRINT
          const clone = originalTable.cloneNode(true) as HTMLElement;
          
          // Remove sticky positioning which confuses html2canvas
          ...
          // --- ENHANCEMENTS FOR PDF READABILITY ---
          // 1. Expand truncated text (Doctor Names) and allow wrapping
          ...
          // 2. Increase Font Size specifically for Doctor Names and important text
          // Targeting the bold text which usually contains names
          const nameElements = clone.querySelectorAll('.font-bold');
          nameElements.forEach(el => {
              const currentFontSize = window.getComputedStyle(el).fontSize;
              // If it's small (like 14px or less), bump it up significantly
          ...
          // 3. Enlarge Avatars (Colored Circles)
          ...
          avatars.forEach(el => {
               ...
               (el as HTMLElement).style.minWidth = '24px'; // Prevent shrinking in flex
               (el as HTMLElement).style.fontSize = '12px';
               (el as HTMLElement).style.marginRight = '8px'; // More space after avatar
          });

          // 4. Adjust Cell Padding and Height
          const cells = clone.querySelectorAll('td, th');
          cells.forEach(el => {
              (el as HTMLElement).style.padding = '10px';
              (el as HTMLElement).style.height = 'auto'; // Allow growth
          });
          
          // 5. Enhance Headers (Dates)
          ...
          const canvas = await html2canvas(printContainer, {
              ...
              width: 2200, // Force capture width
  ...
  const renderCell = (day: DayOfWeek, period: Period, location: string) => {
    ...
    // Closed logic for Box 1/2/3 Monday Morning
    ...
    // --- STYLE LOGIC BASED ON COLOR MODE ---
    ...
    if (doc) {
        bgClass = "bg-white";
        // Convert tailwind color class 'bg-blue-100' -> border-blue-500 for the left strip
        // Simple mapping heuristic or use default blue
    ...
    if (colorMode === 'ACTIVITY') {
        if (slot.type === SlotType.ACTIVITY) {
            ...
            if (def) {
                // Parse the bg color from definition (e.g. "bg-orange-100 text-orange-800")
                const colorParts = def.color.split(' ');
                bgClass = colorParts.find(c => c.startsWith('bg-')) || 'bg-gray-100';
                // Make border slightly darker version of bg or standard
        ...
        {doc ? (
            <>
                <div className="flex items-center space-x-2">
                     <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-bold ${doc.color} shadow-sm border border-black/5`}>
                ...
                {/* In Activity Mode, show activity name if it's the main focus */}
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto items-center">
            ...
            <div className="flex items-center space-x-2 bg-white p-2 rounded-lg shadow-sm border border-slate-200">
                ...
                <input 
                    type="date"
                    className="border-none text-slate-700 font-medium text-sm focus:ring-0 bg-transparent"
                        ...
                        <React.Fragment key={doc.id}>
                            <tr>
                                <td rowSpan={2} className="p-3 border-r border-b bg-slate-50 sticky left-0 z-10 text-center shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                                    <div className={`w-8 h-8 rounded-full mx-auto flex items-center justify-center text-xs font-bold ${doc.color}`}>
                                        {doc.name.substring(0,2)}
                                    </div>
